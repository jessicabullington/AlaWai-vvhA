...
ggplot(data, aes(x = Site, y = SampleDepth)) + 
  geom_tile(aes(fill = vvhA)) +
  scale_fill_viridis(name = "Raw vvhA copies/mL", na.value = 'white', option="inferno", direction = -1,
                     limits = c(0,600),breaks = c(200, 400, 600))  +
  facet_grid(Date ~.) 
  
  theme(axis.text.y = element_text(size = 10, angle = 0, hjust = 1, vjust=0.2), axis.title.y = element_blank(),
